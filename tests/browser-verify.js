...
(async () => {
  ...
  assert.equal(await page.locator('[data-balloon-qty="latex"]').inputValue(), "4");
  assert.equal(await page.locator('[data-balloon-qty="mylar"]').inputValue(), "2");
  assert.match(await page.locator("[data-balloon-total]").innerText(), /\$18/);
  await page.locator('[data-balloon-step="latex:1"]').click();
  assert.match(await page.locator("[data-balloon-total]").innerText(), /\$20/);
  await page.locator('[data-cart-add="Balloon Bouquet"]').click();
  await page.goto(`${base}/cart.html`, { waitUntil: "domcontentloaded" });
  assert.match(await page.locator(".cart-line-instructions").innerText(), /Latex balloons: 5/);
  assert.match(await page.locator(".cart-line-instructions").innerText(), /Mylar balloons: 2/);
  assert.match(await page.locator(".cart-line-price").innerText(), /\$20/);
  ...
  await page.locator('[data-addon-step="5:1"]').click();
  assert.equal(await latexAddon.inputValue(), "3");
  await page.locator('[data-addon-add="5"]').click();
  assert.match(await page.locator(".cart-toast").innerText(), /3 × Latex balloon — shop supplied, helium included/i);
